...
open class CBORDecoder {
	...
	fileprivate var istream : CBORInputStream
	...
	fileprivate func readUInt<T: UnsignedInteger>(_ n: Int) throws -> T {
        let bytes = Array(Array(try istream.popBytes(n)).reversed())
        let result = UnsafePointer(bytes).withMemoryRebound(to: T.self, capacity: 1) {
            $0.pointee
        }
        return result
        
//		return UnsafePointer<T>(Array(try istream.popBytes(n)).reversed()).pointee
	}
...
/*
		case 0xf9: return CBOR.half(loadFromF16(UnsafePointer<UInt16>(Array(try istream.popBytes(2)).reversed())))
		case 0xfa: return CBOR.float(UnsafePointer<Float32>(Array(try istream.popBytes(4)).reversed()).pointee)
		case 0xfb: return CBOR.double(UnsafePointer<Float64>(Array(try istream.popBytes(8)).reversed()).pointee)
*/
        ...
        case 0xfa:
            let bytes = Array(Array(try istream.popBytes(4)).reversed())
        ...
        case 0xfb:
            let bytes = Array(Array(try istream.popBytes(8)).reversed())
